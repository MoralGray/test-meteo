...
function MenubarCheckboxItem({
    ...
    return (
        <MenubarPrimitive.CheckboxItem
            ...
            data-inset={inset}
            className={cn(
                'relative flex cursor-default items-center gap-2 rounded-none py-2 pe-28 ps-8 text-xs outline-hidden select-none focus:bg-accent focus:text-accent-foreground focus:**:text-accent-foreground data-inset:ps-8 data-disabled:pointer-events-none [&_svg]:pointer-events-none [&_svg]:shrink-0',
                className
            )}
            checked={checked}
            {...props}
...
function MenubarRadioItem({
    ...
    return (
        <MenubarPrimitive.RadioItem
            ...
            data-inset={inset}
            className={cn(
                "relative flex cursor-default items-center gap-2 rounded-none py-2 pe-2 ps-8 text-xs outline-hidden select-none focus:bg-accent focus:text-accent-foreground focus:**:text-accent-foreground data-inset:ps-8 data-disabled:pointer-events-none data-disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4",
                className
            )}
            {...props}
        >
...
function MenubarLabel({
    ...
    return (
        <MenubarPrimitive.Label
            ...
            data-inset={inset}
            className={cn('px-2 py-2 text-xs data-inset:ps-8', className)}
            {...props}
        />
    );
}
...
function MenubarSubTrigger({
    ...
    return (
        <MenubarPrimitive.SubTrigger
            ...
            data-inset={inset}
            className={cn(
                "flex cursor-default items-center gap-2 rounded-none px-2 py-2 text-xs outline-none select-none focus:bg-accent focus:text-accent-foreground data-inset:ps-8 data-open:bg-accent data-open:text-accent-foreground [&_svg:not([class*='size-'])]:size-4",
                className
            )}
            {...props}
        >
            {children}
            <RiArrowRightSLine className="rtl:rotate-180 ms-auto size-4" />
        </MenubarPrimitive.SubTrigger>
    );
}
